...
local function get_labels(sublist, to_eol_3f)
  ...
  elseif (not opts.labels or empty_3f(opts.labels)) then
    ...
    local _315_ = sublist["autojump?"]
    if (_315_ == true) then
      return opts.safe_labels
    elseif (_315_ == false) then
      return opts.labels
    elseif (_315_ == nil) then
...
local function set_label_states_for_sublist(sublist, _328_)
  local _arg_329_ = _328_
  local group_offset = _arg_329_["group-offset"]
  ...
  for i, target in ipairs(sublist) do
    ...
    if target.label then
      ...
      elseif (i <= primary_end) then
        _332_ = "active-primary"
      else
        _332_ = "active-secondary"
      end
    else
    _332_ = nil
    end
    target["label-state"] = _332_
  end
  return nil
end
...
local function set_shortcuts_and_populate_shortcuts_map(targets)
  ...
  do
    ...
    for ch2, _ in pairs(targets.sublists) do
      local _335_, _336_ = ch2, true
      if ((nil ~= _335_) and (nil ~= _336_)) then
        local k_10_auto = _335_
        local v_11_auto = _336_
  ...
  for _, _338_ in ipairs(targets) do
...
local function set_beacon(_342_, _repeat)
  local _arg_343_ = _342_
  local target = _arg_343_
  local label = _arg_343_["label"]
  local label_state = _arg_343_["label-state"]
  local overlapped_3f = _arg_343_["overlapped?"]
  local _arg_344_ = _arg_343_["pair"]
  local ch1 = _arg_344_[1]
  local ch2 = _arg_344_[2]
  local _arg_345_ = _arg_343_["pos"]
  local _ = _arg_345_[1]
  local col = _arg_345_[2]
  local left_bound = _arg_345_[3]
  local right_bound = _arg_345_[4]
  local shortcut_3f = _arg_343_["shortcut?"]
  local squeezed_3f = _arg_343_["squeezed?"]
  local to_eol_3f = ((ch1 == "\n") and (ch2 == ""))
  local ch10
  if to_eol_3f then
    ch10 = "\13"
  ...
  local function _348_(_241)
    local function _350_()
      local t_349_ = opts.substitute_chars
      if (nil ~= t_349_) then
        t_349_ = (t_349_)[_241]
      end
      return t_349_
    end
    return (_350_() or _241)
  end
  local _let_347_ = map(_348_, {ch10, ch2})
  ...
  if (_repeat == "instant-unsafe") then
    ...
    elseif (_352_ == "active-primary") then
      ...
      elseif _repeat then
        ...
        target.beacon = {_356_, {shortcut_24}}
    ...
    elseif (_352_ == "active-secondary") then
      ...
      elseif _repeat then
        ...
        target.beacon = {_362_, {distant_label_24}}
      elseif overlapped_3f then
...
local function set_beacons(target_list, _367_)
  local _arg_368_ = _367_
  local _repeat = _arg_368_["repeat"]
...
local function light_up_beacons(target_list, ghost_beacons_3f, _3fstart_idx)
  for i = (_3fstart_idx or 1), #target_list do
    ...
    local _371_ = target.beacon
    if ((type(_371_) == "table") and (nil ~= (_371_)[1]) and (nil ~= (_371_)[2]) and true) then
      local offset = (_371_)[1]
      local chunks = (_371_)[2]
      local _3fleft_off_3f = (_371_)[3]
      ...
      if ghost_beacons_3f then
        ...
        if (col_delta > min_col_delta) then
          local chunks0
          do
            local _377_ = target["label-state"]
            if (_377_ == "active-primary") then
              chunks0 = {{target.label, hl.group["label-overlapped"]}}
            elseif (_377_ == "active-secondary") then
...
local function get_target_with_active_primary_label(target_list, input)
  local res = nil
  for _, _382_ in ipairs(target_list) do
    local _each_383_ = _382_
    local target = _each_383_
    local label = _each_383_["label"]
    local label_state = _each_383_["label-state"]
...
local function ignore_input_until_timeout(char_to_ignore)
  local _385_ = opts.jump_to_unique_chars
  if ((type(_385_) == "table") and (nil ~= (_385_).safety_timeout)) then
    local timeout = (_385_).safety_timeout
    local _386_ = get_input(timeout)
    if (nil ~= _386_) then
      local input = _386_
...
sx.go = function(self, reverse_3f, x_mode_3f, repeat_invoc, cross_window_3f)
  ...
  if cold_repeat_3f then
    ...
    reverse_3f0 = _391_(self.state.cold["reverse?"])
  else
    reverse_3f0 = reverse_3f
  end
  local x_mode_3f0
  ...
  _3ftarget_windows = (_395_() or _396_())
  ...
  if (nil ~= _463_) then
    ...
    elseif (nil ~= _465_) then
      ...
      if not instant_repeat_3f then
        local _476_ = targets
        populate_sublists(_476_)
        set_labels(_476_, to_eol_3f)
        set_label_states(_476_)
      end
      if (new_search_3f and not to_eol_3f) then
        do
          local _478_ = targets
          set_shortcuts_and_populate_shortcuts_map(_478_)
          set_beacons(_478_, {["repeat"] = nil})
        end
        if not (cold_repeat_3f or instant_repeat_3f) then
          grey_out_search_area(reverse_3f0, _3ftarget_windows)
        end
      ...
      _481_ = (prev_in2 or _482_() or _483_() or _484_())
      if (nil ~= _481_) then
        local in2 = _481_
        local _486_
        do
        ...
        if ((type(_486_) == "table") and ((type((_486_).pair) == "table") and true and (nil ~= ((_486_).pair)[2]))) then
          ...
          _490_ = (_492_() or get_sublist(targets, in2) or _494_())
          if ((type(_490_) == "table") and (nil ~= (_490_)[1]) and ((_490_)[2] == nil)) then
            local only = (_490_)[1]
            do
              if dot_repeatable_op_3f then
                set_dot_repeat(replace_keycodes(get_plug_key("sx", reverse_3f0, x_mode_3f0, "dot")))
            ...
            doau_when_exists("LightspeedLeave")
            return nil
          elseif ((type(_490_) == "table") and (nil ~= (_490_)[1])) then
            local first = (_490_)[1]
            local sublist = _490_
            ...
            _503_ = (_504_() or get_last_input(sublist, inc(curr_idx)) or _505_())
            if ((type(_503_) == "table") and (nil ~= (_503_)[1]) and (nil ~= (_503_)[2])) then
              local in3 = (_503_)[1]
...
local function set_plug_keys()
  ...
  for _, _531_ in ipairs(plug_keys) do
  ...
  for _, _533_ in ipairs({{"<Plug>Lightspeed_dotrepeat_s", "sx:go(false, false, 'dot')"}, {"<Plug>Lightspeed_dotrepeat_S", "sx:go(true, false, 'dot')"}, {"<Plug>Lightspeed_dotrepeat_x", "sx:go(false, true, 'dot')"}, {"<Plug>Lightspeed_dotrepeat_X", "sx:go(true, true, 'dot')"}, {"<Plug>Lightspeed_dotrepeat_f", "ft:go(false, false, 'dot')"}, {"<Plug>Lightspeed_dotrepeat_F", "ft:go(true, false, 'dot')"}, {"<Plug>Lightspeed_dotrepeat_t", "ft:go(false, true, 'dot')"}, {"<Plug>Lightspeed_dotrepeat_T", "ft:go(true, true, 'dot')"}}) do
...
local function set_default_keymaps()
  ...
  for _, _535_ in ipairs(default_keymaps) do
    local _each_536_ = _535_
    local mode = _each_536_[1]
    local lhs = _each_536_[2]
    local rhs = _each_536_[3]
